...
class Network(object):
    ...
    def feedfowrd(self, a):
        # retorna a saída da rede se 'a' for input
        for b, w in zip(self.biases, self.wights):
    ...
    def backprop(self, x, y):
        ...
        # lista para armazenar todas as ativações
        activations = [x]

        # lista para armazenar todos os vetores z
        zs = []
    ...
    def evaluate(self, test_data):
        ...
        test_results = [(np.argmax(self.feedfowrd(x)), y) for (x, y) in test_data]
...
# função para retornar as derivadas da fução sigmóide
def sigmoid_prime(z):
